...
import {HttpService} from '../src/lib/services/http.service';
...
>(
  ...
  const {serverMocks = {}, responseMocks = {}} = serviceMocks;
...
describe('http', () => {
  it('instances', () => {
    ...
    //@ts-ignore
    expect(service.responseService instanceof MockBuilder).equal(
      true,
      '@lib/response'
    );
  });
  ...
  it('#handler (error)', () => {
    ...
    const extractQueryArg = serviceTestea
      .getResult('extractQuery')
      .getArgFirst();
    const extractBodyArg = serviceTestea.getResult('extractBody').getArgFirst();
    const getMethodArgs = serviceTestea.getResult('getMethod').getArgs();
    const getEndpointArg = serviceTestea.getResult('getEndpoint').getArgFirst();
    const executeArgs = serviceTestea.getResult('execute').getArgs();
  ...
  it('#execute (has middleware, no data)', () => {
    ...
    const r = service.execute(
      [(req, res, next) => next(), (...args: any[]) => args],
      {data: {}} as any,
      {} as any
    );
    const req = r[0];
  ...
  it('#execute (has middleware, has data)', () => {
    ...
    const r = service.execute(
      [(req, res, next) => next({a: 1}), (...args: any[]) => args],
      {data: {}} as any,
      {} as any
    );
    const req = r[0];
