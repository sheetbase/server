...
import {MiddlewareService} from '../src/lib/services/middleware.service';
...
>(
  ...
  const {serverMocks = {}, apiKeyMocks = {}} = serviceMocks;
...
describe('api-key', () => {
  it('instances', () => {
    ...
    // @ts-ignore
    expect(service.apiKeyService instanceof MockBuilder).equal(
      true,
      '@lib/api-key'
    );
  });
  ...
  it('failed (#render)', () => {
    ...
    const {'@lib/api-key': apiKeyTesting} = serviceRewiring.getMockedServices();
