...
import {ServiceStubing, rewireService} from '@lamnhan/testea';

import {ServerService} from '../src/lib/services/server.service';

function setup<ServiceStubs extends ServiceStubing<ServerService>>(
  serviceStubs?: ServiceStubs
) {
  return rewireService(ServerService, undefined, serviceStubs).getResult();
}
...
describe('server', () => {
  ...
  it('props', () => {
    ...
    // @ts-ignore
    expect(service.methods).eql(['get', 'post', 'put', 'patch', 'delete']);
  });
  ...
  it('#getMethods', () => {
    ...
    expect(r).eql(['get', 'post', 'put', 'patch', 'delete']);
  });
  ...
  it('#getRoute (not exists)', () => {
    ...
    const handler = r[0];
    const isRouteDisabledArgs = serviceTestea
      .getResult('isRouteDisabled')
      .getArgs();
  ...
  it('#addRoute', () => {
    ...
    const setRouteHandlerArgs = serviceTestea
      .getResult('setRouteHandler')
      .getArgs();
    const setRouteMiddlewaresArgs = serviceTestea
      .getResult('setRouteMiddlewares')
      .getArgs();

    expect(setRouteHandlerArgs).eql(['get', '/', 'handler']);
    expect(setRouteMiddlewaresArgs).eql(['get', '/', []]);
  });
  ...
  it('#addRouteAll', () => {
    ...
    const setRouteHandlerAllArgs = serviceTestea
      .getResult('setRouteHandlerAll')
      .getArgs();
    const setRouteMiddlewaresAllArgs = serviceTestea
